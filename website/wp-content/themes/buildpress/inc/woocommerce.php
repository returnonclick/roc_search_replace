...
if ( is_woocommerce_active() ) {
	 ...
	 * @return array
	 */
	function pt_woo_body_class( $classes ) {
	...
	add_filter( 'body_class', 'pt_woo_body_class', 11 );
	 ...
	 * @return array
	 */
	function pt_output_related_products_args( $args ) {
		$args[ 'posts_per_page' ] = 4;
		$args[ 'columns' ]        = 4;

		return $args;
	}
	add_filter( 'woocommerce_output_related_products_args', 'pt_output_related_products_args' );

	/**
	 * Get the position of the sidebar for the shop pages, conditionally for the single product
	 */
	function pt_get_shop_sidebar() {
		if ( is_product() ) {
